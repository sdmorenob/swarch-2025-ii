...
const TagsPage: React.FC = () => {
      ...
      const normalized = Array.isArray(data)
        ...
        : (data as any)?.items && Array.isArray((data as any).items)
          ...
          : (data as any)?.results && Array.isArray((data as any).results)
            ? (data as any).results
            : (data as any)?.tags && Array.isArray((data as any).tags)
              ? (data as any).tags
              : [];
      setTags(normalized);
    } catch (err: any) {
  ...
  const handleSubmit = async () => {
    try {
      const isUpdate = Boolean(editingTag);
      if (isUpdate && editingTag) {
      ...
      handleCloseDialog();
      setError(null);
      setSuccessMessage(isUpdate ? 'Etiqueta actualizada correctamente' : 'Etiqueta creada correctamente');
      setSuccessOpen(true);
    } catch (err: any) {
      const status = err?.response?.status;
      if (status === 409) {
