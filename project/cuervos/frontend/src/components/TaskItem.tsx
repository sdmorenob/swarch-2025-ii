...
const TaskItem: React.FC<TaskItemProps> = ({ task, onUpdate, onDelete, onEdit, categories, tags }) => {
  ...
  const [selectedTagIds, setSelectedTagIds] = useState<string[]>((task.tags || []).map(t => String(t.id)));
  ...
  const handleToggleComplete = async () => {
    try {
      ...
      await apiService.updateTask(task.id, { completed: updatedTask.completed });
      onUpdate(updatedTask as TaskType);
    } catch (error) {
  ...
  const handleCancel = () => {
    ...
    setSelectedCategoryId(task.category?.id || '');
        ...
        <Box display="flex" alignItems="flex-start" gap={1}>
          ...
          <Box flex={1}>
            ...
            <Box display="flex" alignItems="center" gap={1} flexWrap="wrap" mb={1}>
              <Chip
                icon={<Flag />}
                label={`${getPriorityIcon(task.priority)} ${task.priority.toUpperCase()}`}
                size="small"
