...
export const getServerSideProps: GetServerSideProps<Props> = async (context) => {
  ...
  // Derivar URL pública de SPA desde entorno o cabeceras del proxy (evita fallback a localhost)
  const protoHeader = context.req.headers['x-forwarded-proto'] || context.req.headers['x-forwarded-protocol'];
  const proto = Array.isArray(protoHeader) ? protoHeader[0] : protoHeader;
  const hostHeader = context.req.headers['x-forwarded-host'] || context.req.headers['host'];
  const host = Array.isArray(hostHeader) ? hostHeader[0] : hostHeader;
  const inferredPublic = proto && host ? `${proto.split(',')[0]}://${host.split(',')[0]}` : undefined;

  const webUrlPublic = process.env.WEB_URL_PUBLIC
    || process.env.NEXT_PUBLIC_WEB_URL_PUBLIC
    || inferredPublic
    || 'http://localhost:8080';

  // Prefijo del SPA (en EKS está expuesto bajo /micro)
  const webBasePathRaw = process.env.WEB_BASE_PATH || '/micro';
  const webBasePath = webBasePathRaw.startsWith('/') ? webBasePathRaw : `/${webBasePathRaw}`;
  const webBasePathClean = webBasePath === '/' ? '' : webBasePath.replace(/\/$/, '');
  const spaBase = `${webUrlPublic}${webBasePathClean}`;
  ...
  const destination = webUp ? `${spaBase}${destPath}` : `${spaBase}/login`;
...
export default function Home({ destination, webUp, webUrlPublic, spaBase, autoRedirect, to }: Props) {
  ...
  const registerUrl = `${spaBase}/register`;
        ...
        <div className="hero-right">
          ...
          <div className="status">
            ...
            <div className="status-details">
              ...
              <div>
                SPA base path: <code>{spaBase}</code>
              </div>
              <div>
