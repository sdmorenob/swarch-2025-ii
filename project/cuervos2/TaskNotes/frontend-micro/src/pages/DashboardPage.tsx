...
const DashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    if (socketCtx) {
      ...
      socketCtx.on('task_updated', handleTaskUpdated as any);
      socketCtx.on('task_deleted', handleTaskDeleted as any);
      socketCtx.on('note_updated', handleNoteUpdated as any);

      return () => {
        socketCtx.off('task_updated', handleTaskUpdated as any);
        socketCtx.off('task_deleted', handleTaskDeleted as any);
          ...
          {taskPreview && (
            <Box>
              ...
              <Box sx={{ mt: 2, pt: 2, borderTop: 1, borderColor: 'divider' }}>
                ...
                <br />
                <Typography variant="caption" color="text.secondary">
                  Updated: {new Date(taskPreview.updated_at || taskPreview.created_at).toLocaleString()}
                </Typography>
              </Box>
            </Box>
          )}
