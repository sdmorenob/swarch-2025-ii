...
# Prefijos internos esperados por cada servicio
...
# Alias para llamadas con prefijo /api (ej. /api/auth/register) que llegan desde frontends
@app.api_route("/api/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH"])
async def api_prefixed_proxy(request: Request, path: str):
    """Proxy genérico que elimina el prefijo /api y enruta al servicio según el primer segmento."""
    # Si viene vacío, redirigir al health del gateway
    if not path:
        return {"status": "ok", "via": "api-prefix"}

    # Determinar servicio y ruta interna
    parts = path.split("/", 1)
    raw_service = parts[0].strip()
    remaining = parts[1] if len(parts) > 1 else ""

    # Normalizar nombres con guion a underscore para que coincidan con SERVICE_POOLS
    service = raw_service.replace("-", "_")

    # Determinar prefijo interno del servicio
    prefix = SERVICE_PATH_PREFIXES.get(service)
    if prefix is None:
        raise HTTPException(status_code=404, detail=f"Servicio {service} no encontrado")

    # Para servicios protegidos, validar token y aplicar policies/inyección de headers
    protected_services = {"tasks", "notes", "tags", "categories", "user_profile", "search"}
    token_payload = None
    if service in protected_services:
        credentials = await security(request)
        token_payload = await verify_token(credentials)  # lanza 401 si falta o es inválido
        enforce_policies(token_payload, service.replace("_", "-"), request.method)

    if remaining:
        forward_path = f"{prefix}/{remaining}" if prefix else f"/{remaining}"
        if request.url.path.endswith("/") and not forward_path.endswith("/"):
            forward_path += "/"
    else:
        if prefix and request.url.path.endswith("/"):
            forward_path = prefix if prefix.endswith("/") else f"{prefix}/"
        else:
            forward_path = prefix if prefix else ("/" if request.url.path.endswith("/") else "")

    if service not in SERVICE_POOLS:
        raise HTTPException(status_code=404, detail=f"Servicio {service} no encontrado")

    proxied = await proxy_request(request, service, forward_path, token_payload)
    return Response(content=proxied.content, status_code=proxied.status_code, media_type=proxied.headers.get("content-type"))
